...
?>
    ...
    <div class="form-group">
        <div class="col-md-2">
            ...
            <select name="section_id_1" id="section_id_1" class="form-control">
            </select>
        </div>
    </div>
    ...
    <div class="form-group">
        <div class="col-md-2">
            <label class="control-label" style="margin-bottom: 5px;"><?php echo get_phrase('select_section');?></label>
            <select name="section_id_2" id="section_id_2" class="form-control">
            </select>
        </div>
    </div>
    ...
    <center>
        <button
                ...
                type="button"
                style="margin:10px;"
                onclick="get_students_to_promote('<?php echo $running_year;?>')">
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $('#from_class_id').on('change',function(){
            var from_class_id = $(this).val();
            if(from_class_id){
                $.ajax({
                    url: '<?php echo base_url();?>index.php?admin/promote_get_section_1/',
                    data:'class_id='+from_class_id,
                    type: "POST",
                    success:function(data){
                        $('#section_id_1').html('<option value="">Select section</option>');
                        var dataObj = jQuery.parseJSON(data);
                        if(dataObj){
                            $(dataObj).each(function(){
                                var option = $('<option />');
                                option.attr('value', this.section_id).text(this.name);
                                $('#section_id_1').append(option);
                            });
                        }else{
                            $('#section_id_1').html('<option value="">section not available</option>');
                        }
                    }
                });
            }else{
                $('#section_id_1').html('<option value="">Select from class first</option>');
            }
        });

        $('#to_class_id').on('change',function(){
            var to_class_id = $(this).val();
            if(to_class_id){
                $.ajax({
                    url: '<?php echo base_url();?>index.php?admin/promote_get_section_2/',
                    data:'class_id='+to_class_id,
                    type: "POST",
                    success:function(data) {
                        $('#section_id_2').html('<option value="">Select section</option>');
                        var dataObj = jQuery.parseJSON(data);
                        if(dataObj){
                            $(dataObj).each(function(){
                                var option = $('<option />');
                                option.attr('value', this.section_id).text(this.name);
                                $('#section_id_2').append(option);
                            });
                        }else{
                            $('#section_id_2').html('<option value="">section not available</option>');
                        }
                    }
                });
            }else{
                $('#section_id_2').html('<option value="">Select to class first</option>');
            }
        });
    ...
    function get_students_to_promote(running_year)
    {
        ...
        section_id_1    = $("#section_id_1").val();
        section_id_2    = $("#section_id_2").val();
        ...
        if (section_id_1 === "" || section_id_2 === "") {
            toastr.error("<?php echo get_phrase('select_section_for_promotion_to_and_from');?>");
            return false;
        }

        $.ajax({
            url: '<?php echo base_url();?>index.php?admin/get_students_to_promote/' + from_class_id + '/' + to_class_id + '/' + section_id_1 + '/' + section_id_2 + '/' + running_year + '/' + promotion_year,
