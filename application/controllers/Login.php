...
class Login extends CI_Controller {
    ...
    function validate_login($username = '', $password = '') {
        ...
        $get_student_regno = array('enroll_code' => $username);
        $get_student_pass  = array('password' => $password);

        $query = $this->db->get_where('enroll', $get_student_regno);
        if ($query->num_rows() > 0) {

            $output1    = $query->row();
            $student_id = $output1->student_id;
            $query2     = $this->db->get_where('student', array('student_id' => $student_id, 'password' => $password));
