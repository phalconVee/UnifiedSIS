...
class Student extends CI_Controller
{
    ...
    function dashboard()
    {
        ...
        $student_id = $this->db->get_where('student', array(
            'student_id' => $this->session->userdata('student_id')
        ))->row()->student_id;

        $status = $this->crud_model->getStudentStatus($student_id);
    ...
    function student_marksheet($student_id = '', $pin = '') {
        ...
        $result_checker_settings = $this->db->get_where('settings', array('type' => 'scratch_card_active'))->row()->description;

        if($result_checker_settings == 1 && $pin == '') {
            redirect(base_url() . 'index.php?student/result_checker/' . $student_id, 'refresh');
        }
    ...
    function result_checker($student_id='', $param2='') 
    {
        if ($this->session->userdata('student_login') != 1)
            redirect('login', 'refresh');

        if($param2 == 'check_result'){

            $pin = $this->input->post('pin');

            $validate = $this->db->get_where('cards', array('pin' => $pin));

            if($validate->num_rows() > 0){
                $row = $validate->row();
                $hashed_string = md5($row->pin); //use thi to encrypt pin to avoid display in url  when redirected
                redirect(base_url() . 'index.php?student/student_marksheet/' . $student_id . '/'. $hashed_string, 'refresh');
            }
            else{
                $this->session->set_flashdata('error_message' , get_phrase('this_pin_does_not_exist'));
            }
        ...
        $page_data['page_name']  =   'student_result_checker';
        $page_data['page_title'] =   get_phrase('result_checker_for') . ' ' . $student_name . ' (' . get_phrase('class') . ' ' . $class_name . ')';
    ...
    function pay_with_paystack($param1 = "", $param2 = "") {

        $page_data['page_name']    = 'pay_with_paystack';
        $page_data['page_title']   = get_phrase('pay_with_paystack');
        $page_data['student_id']   = $param1;
        $page_data['invoice_id']   = $param2;
        $page_data['merch_txnref'] = 'STP-' . strtoupper(uniqid());
        $this->load->view('backend/index', $page_data);
    }

    /*function pay_with_paystac($param1 = "", $param2 = "") {

        if(isset($_POST)){

            //collect all posted params needed to be parsed to paytsack via JS
